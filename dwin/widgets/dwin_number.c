...
struct dwin_number *dwin_number_create(struct dwin_page *page, rt_uint16_t addr, enum dwin_number_type type)
{
    struct dwin_number *number = RT_NULL; 
    
    RT_ASSERT(page != RT_NULL); 
    
    number = (struct dwin_number *)rt_malloc(sizeof(struct dwin_number)); 
    if(number == RT_NULL)
    {
